...
module.exports = {
  async getAll(ctx) {
    try {
      const count = await strapi.db.query("api::antonym.antonym").count();
      const data = await strapi.db.query("api::antonym.antonym").findMany();
  ...
  async get(ctx) {
    try {
      ...
      const count = await strapi.db
        .query("api::antonym.antonym")
        .count({ where: { ind1: { $startsWith: key } } });

      const data = await strapi.db.query("api::antonym.antonym").findMany({
        where: { ind1: { $startsWith: key } },
        offset,
        limit,
      });
